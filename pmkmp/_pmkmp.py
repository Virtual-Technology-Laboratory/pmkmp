...
# this is a port of Matteo Niccoli BSD licensed
# matlab package to Python/matplotlib

# Ported by rogerlew@gmail.com
# Port licensed under BSD

"""
Rainbow, or spectrum color schemes are considered a poor
...
def _matlabArray_to_TupleLists(stringArray):
    unpack = lambda w : float(''.join(c for c in w if c in '.0123456789'))
    
    L = []

    for line in stringArray.splitlines():
        L.append(tuple([unpack(w) for w in line.split()] + [1.0]))
...
_Edge = _matlabArray_to_TupleLists(_Edge)
_Edge_x = np.linspace(0, 1, len(_Edge))
Edge = _buildLinearSegmentedColormap(_Edge_x, _Edge, "IsoL")
_cms_dict["Edge"] = Edge

_IsoL =   """[0.9102    0.2236    0.8997
...
_IsoL = _matlabArray_to_TupleLists(_IsoL)
_IsoL_x = np.linspace(0, 1, len(_IsoL))
IsoL = _buildLinearSegmentedColormap(_IsoL_x, _IsoL, "IsoL")
_cms_dict["IsoL"] = IsoL

_CubicL =  """[0.4706         0    0.5216;
...
_CubicL = _matlabArray_to_TupleLists(_CubicL)
_CubicL_x = np.linspace(0, 1, len(_CubicL))
CubicL = _buildLinearSegmentedColormap(_CubicL_x, _CubicL, "CubicL")
_cms_dict["CubicL"] = CubicL

_CubicYF = """[0.5151    0.0482    0.6697
...
_CubicYF = _matlabArray_to_TupleLists(_CubicYF)
_CubicYF_x = np.linspace(0, 1, len(_CubicYF))
CubicYF = _buildLinearSegmentedColormap(_CubicYF_x, _CubicYF, "CubicYF")
_cms_dict["CubicYF"] = CubicYF


_LinearL = """ [0.0143	0.0143	0.0143
...
_LinearL = _matlabArray_to_TupleLists(_LinearL)
_LinearL_x = np.linspace(0, 1, len(_LinearL))
LinearL = _buildLinearSegmentedColormap(_LinearL_x, _LinearL, "LinearL")
_cms_dict["LinearL"] = LinearL

_LinLhot =  """[0.0225	0.0121	0.0121
...
_LinLhot = _matlabArray_to_TupleLists(_LinLhot)
_LinLhot_x = np.linspace(0, 1, len(_LinLhot))
LinLhot = _buildLinearSegmentedColormap(_LinLhot_x, _LinLhot, "LinLhot")
_cms_dict["LinLhot"] = LinLhot

_IsoAZ = """[1.0000	0.2627	1.0000
...
_IsoAZ = _matlabArray_to_TupleLists(_IsoAZ)
_IsoAZ_x = np.linspace(0, 1, len(_IsoAZ))
IsoAZ = _buildLinearSegmentedColormap(_IsoAZ_x, _IsoAZ, "IsoAZ")
_cms_dict["IsoAZ"] = IsoAZ

_IsoAZ180 = """ [0.8658	0.5133	0.6237
...
_IsoAZ180 = _matlabArray_to_TupleLists(_IsoAZ180)
_IsoAZ180_x = np.linspace(0, 1, len(_IsoAZ180))
IsoAZ180 = _buildLinearSegmentedColormap(_IsoAZ180_x, _IsoAZ180, "IsoAZ180")
_cms_dict["IsoAZ180"] = IsoAZ180

_Swtth = """ [1.0000	0.5395	1.0000
...
_Swtth = _matlabArray_to_TupleLists(_Swtth)
_Swtth_x = np.linspace(0, 1, len(_Swtth))
Swtth = _buildLinearSegmentedColormap(_Swtth_x, _Swtth, "Swtth")
_cms_dict["Swtth"] = Swtth

for name, cm in _cms_dict.items():
    mpl.cm.register_cmap(name, cm, data=cm._segmentdata)
